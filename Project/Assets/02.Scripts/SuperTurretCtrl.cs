...
public class SuperTurretCtrl : MonoBehaviour
{
  ...
  //  private float traceDist = 30.0f;//추적 사정거리
    private float attackDist = 16.50f;//공격 사정 거리
    ...
    void start()
    {
        TurretCtrl1 = GameObject.Find("SuperTurret");
    }
    void Update()
    {
    ...
    IEnumerator CheckMonsterState()//일정한 간격으로 몬스터의 행동 상태를 체크하고 몬스터스테이트 값 변경
    {
        while (!isDie)
        {
            ...
            yield return new WaitForSeconds(0.02153f);//몇발쏠지 여기서 결정
            //몬스터와 플레이어 사이의 거리 측정
            float dist = Vector3.Distance(playerTr.position, monsterTr.position);

            if (dist <= attackDist && ftime > 1.5f && Damage.GetComponent<PlayerCtrl>().Kanchi == 1)
            {//공격거리범위 이내로 들어왔는지 확인
