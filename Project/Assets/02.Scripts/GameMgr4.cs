...
public class GameMgr4 : MonoBehaviour
{
    ...
    public float createTime = 2.0f;//몬스터를 발생시킬 주기
    public int maxMonster = 6;//몬스터 최대 발생개수
    public bool isGameOver = false;//게임종료 변수여부


    void Start()
    {
        points = GameObject.Find("SpawnPoint4").GetComponentsInChildren<Transform>();//SpawnPoint4를 찾아 하위에 있는 모든 트랜스폼 컴포넌트를 찾아옴
        ...
        for (int i = 0; i < maxMonster; i++)
        {
            ...
            monsterPool[i].name = "Monster_" + i.ToString();//생성한 몬스터의 이름 설정
